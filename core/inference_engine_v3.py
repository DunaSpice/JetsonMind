...
class Phase3InferenceEngine:
    ...
    def __init__(self):
        ...
        # Performance optimization: Add response caching
        self.response_cache = {}
        self.cache_hits = 0
        self.cache_misses = 0
    ...
    def generate_text(self, prompt: str, model: str = None, thinking_mode=None):
        ...
        cache_key = f"{selected_model}:{hash(prompt)}:{thinking_mode}"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(result, list) and len(result) > 0:
                    ...
                    if new_text:
                        response_text = f"🤗 Real AI ({selected_model}): {new_text}"
        ...
        # Existing simulation fallback with caching
        response_text = f"🧠 Generated by {selected_model}: {prompt}... (simulated)"
        self.response_cache[cache_key] = response_text
        return response_text
